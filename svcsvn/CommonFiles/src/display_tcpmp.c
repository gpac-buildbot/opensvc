...
/**
This function writes the output pictures int a yuv file.
*/
void WriteYUV(int PicWidth, int PicHeight, unsigned char *Y, unsigned char *U, unsigned char *V, FILE *file){

	int i;
	int XDIM = PicWidth - 32;	//remove padding
	int YDIM = PicHeight - 32;  //remove padding

	for(i = 0; i < YDIM; i++)
		fwrite(Y + i * PicWidth, 1, XDIM, ptfile);

	for(i = 0; i < (YDIM >> 1); i++)
		fwrite(U + i * (PicWidth >> 1), 1, (XDIM>>1), ptfile);

	for(i = 0; i < (YDIM >> 1); i++)
		fwrite(V + i * (PicWidth >> 1), 1, (XDIM>>1), ptfile); 

}

/**
This function initializes if necessary the output file.
*/
