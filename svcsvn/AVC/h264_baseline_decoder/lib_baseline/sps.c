...
void seq_parameter_set_data ( unsigned char *data, int *position,  SPS *pt_sps)
{
	...
	if( pt_sps -> profile_idc  ==  100  || pt_sps ->  profile_idc  ==  110  ||	
		...
		pt_sps -> profile_idc  ==  44 ||  pt_sps ->  profile_idc  ==  83 ||	pt_sps -> profile_idc  ==  86 ) {
			...
			decode_scaling_matrices(data, position, pt_sps, NULL, 1, pt_sps -> scaling_matrix4, pt_sps -> scaling_matrix8);
	}
...
void seq_parameter_set( unsigned char *data, SPS *sps){
	...
	if (profile_idc && !(constraint & 0xf)){
		...
		short PicWidthInPix = pt_sps -> pic_width_in_mbs;
		short PicHeightInPix = pt_sps -> pic_height_in_map_units;
		short CropLeft = pt_sps -> CropLeft;
		short CropRight = pt_sps -> CropRight; 
		short CropTop = pt_sps -> CropTop;
		short CropBottom = pt_sps -> CropBottom;
		unsigned char MaxDpbWait = pt_sps -> nb_img_disp;
		char MemoryAllocated = pt_sps -> MemoryAllocated;
		...
		pt_sps -> MemoryAllocated = MemoryAllocated;
		sps[0] . MaxSpsId = MaxSpsId;

		seq_parameter_set_data(data, &position, pt_sps);

		//Check video resolution which should not change during the decoding process.
		if(MemoryAllocated && ((PicWidthInPix != pt_sps -> pic_width_in_mbs) || (PicHeightInPix != pt_sps -> pic_height_in_map_units))){
			//Set back previous video resolution 
			pt_sps -> pic_width_in_mbs = PicWidthInPix;
			pt_sps -> pic_height_in_map_units = PicHeightInPix;
			pt_sps -> CropLeft = CropLeft;
			pt_sps -> CropRight = CropRight;
			pt_sps -> CropTop = CropTop;
			pt_sps -> CropBottom = CropBottom;
		}
