...
/**
This functions decodes the level code usefull for the CAVLC.
*/
static __inline void CavlcGetLevelCode(const unsigned char *ai_poData, int *position, int RestBits, int coeff_token_TotalCoeff,
									   int coeff_token_TrailingOnes, short *level)
{
	int levelCode, tmp2, i;
	unsigned int tmp3;
	unsigned int level_prefix = read_levelprefix(ai_poData, position, &RestBits);
	unsigned int suffixLength = ((coeff_token_TotalCoeff > 10) && (coeff_token_TrailingOnes < 3)) ? 1 : 0; 
	...
	for (i = coeff_token_TrailingOnes + 1; i < coeff_token_TotalCoeff ; i++ ) {
		int level_prefix = read_levelprefix(ai_poData, position,(int *) &tmp3);
		RestBits = ( level_prefix == 15 )? 12 : suffixLength;
...
Decodes Run code.
*/
static __inline void CavlcGetRunCode(const unsigned char *ai_poData, int *position, int coeff_token_TotalCoeff, int ai_iMaxNumCoeff,
									 const VLC_TABLES *vlc, short *run)
{
	...
	if(coeff_token_TotalCoeff < 11)	{
		...
		const VLC_TYPE *pstTable = &vlc -> Run_vlc [8 * (zerosLeft - 1)];
		int index;
		int tmp2 = (int)showNbits(ai_poData, *position, 24);
		int ReadLenght = 24 - RUN_VLC_BITS;


		for(; (i < coeff_token_TotalCoeff - 1) && (zerosLeft > 0); i++)	{
			index = (tmp2 >> ReadLenght) & 7;
			ReadLenght -= pstTable [index] . len ;
							   ...
							   const int ai_iN,	const VLC_TABLES *vlc, unsigned char NonZeroCountCache [ RESTRICT ])
{
	...
	*position += tmp ;
	...
	if ( coeff_token_TotalCoeff > 0 ){
		...
		int coeff_token_TrailingOnes = TrailingOnes(coeff_token);
		...
		if ( coeff_token_TrailingOnes < coeff_token_TotalCoeff ){
			CavlcGetLevelCode(ai_poData, position, tmp, coeff_token_TotalCoeff, coeff_token_TrailingOnes, level);
		}

		CavlcGetRunCode(ai_poData, position, coeff_token_TotalCoeff, ai_iMaxNumCoeff, vlc, run);

		{
			unsigned int uiScanTableH = 0xf9be4190;
			unsigned int uiScanTableL = 0xef91b904;
			int iIndex;
			int coeffNum = ( ai_iMaxNumCoeff == 15 )? 0 : -2; 
							 ...
							 const int n, const VLC_TABLES *vlc, unsigned char NonZeroCountCache [ RESTRICT ])
{
	...
	*position += tmp ;
	...
	if ( coeff_token_TotalCoeff > 0 ){
		...
		//Decoding of the number of zéro in the block
		CavlcGetRunCode(ai_poData, position, coeff_token_TotalCoeff, 16, vlc, run);
