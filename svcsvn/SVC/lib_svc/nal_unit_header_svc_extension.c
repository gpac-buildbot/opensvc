...
void init_nal_struct(NAL *Nal, unsigned char MaxLayer){
	...
	memset(&Nal -> LayerIdTable, -1, 8 * sizeof(unsigned char));
...
{	
	...
	*position = 8;
	...
	for ( i = 1; i < 8; i++){
		if ( Nal -> LayerIdTable[i] == -1){
			Nal -> LayerIdTable[i] = Nal -> DqId;
			Nal -> LayerId = i;
			break;
		}
		if(Nal -> LayerIdTable[i] == Nal -> DqId){
			Nal -> LayerId = i;
			break;
		}
		if(Nal -> LayerIdTable[i] > Nal -> DqId){
			Nal -> LayerId = i;
			for ( j = i; j < 7; j ++){
				Nal -> LayerIdTable[i + 1] =  Nal -> LayerIdTable[i];
			}
			Nal -> LayerIdTable[i] = Nal -> DqId;
			break;
		}
...
void NextNalDqId(ConfigSVC *StreamType, NAL *Nal)
{
	...
	if (Nal -> LayerIdTable [0] == -1){
		...
		memcpy(Nal -> LayerIdTable, StreamType -> LayerIdTable, 8 * sizeof(char));
