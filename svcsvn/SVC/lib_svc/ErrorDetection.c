					  ...
					  int *AddressPic, int *xsize, int *ysize)
{
	...
	if (!EndOfSlice && (PrevEndOfSlice || Nal -> ErrorDetected) && Nal -> LastDqId <= Nal -> DqIdToDisplay){
		...
		if(Nal -> LayerIdTable [Mmo -> LayerId] != -1){
			LastStructNal -> DqId = Nal -> LayerIdTable [Mmo -> LayerId];
		}
		else{		
...
void RemoveDisplayError(MMO *Mmo, NAL *Nal)
{
	...
	short DqId = Nal -> LayerIdTable [LayerId];
	...
	if ( DqId > Nal -> DqIdToDisplay){
		...
		if (Found){
			...
			for (i; i < Mmo -> nb_img_display - Mmo -> num_free_poc_address; i++){
