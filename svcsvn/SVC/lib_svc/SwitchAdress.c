...
void switch_layer(NAL *Nal)
{
	...
	if(Nal -> IdrFlag == 1 && (Nal -> ComDqId != Nal -> DqIdWanted)){
		...
		for ( i = 0; i < 8; i++){
			if (SaveDqId == Nal -> LayerIdTable[i]){
				//we find it
				break;
			} else if(i < 7 && SaveDqId > Nal -> LayerIdTable[i] && Nal -> LayerIdTable[i + 1] == -1){
				//we did not find it, but we should wait
				test = 1;
				break;
			}else if (i < 7 && SaveDqId > Nal -> LayerIdTable[i] && SaveDqId < Nal -> LayerIdTable[i + 1]){
		...
		if ( test != 1){
			if (test == 2){
				...
				//we know that the DQId wanted, is between the LayerIdTable [i] and LayerIdTable [i + 1]
				Nal -> DqIdWanted = Nal -> LayerIdTable [i + 1];
			}
			else {
				...
				if (WantedLayer < Nal -> MaxNumLayer){
					Nal -> DqIdWanted = Nal -> LayerIdTable[WantedLayer];
				}else {
					Nal -> DqIdWanted = Nal -> LayerIdTable[Nal -> MaxNumLayer - 1];
				}
			}
	...
	if(Nal -> LayerIdTable[0] == -1){
		Nal -> LayerIdTable [0] = 0;
	}
}
